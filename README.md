...
- **Gallery** — each photo has a full-size and a `-sm` version; the page picks
  the right one per screen. Add a photo by copying a `<figure class="slide">`
  block in `index.html` — the dots and arrows pick it up automatically.
...
- **Mobile** — matches the PDF: single column, sticky "Start an order" bar,
  2-up dish cards, swipeable gallery.
- **Desktop (900px+)** — top navigation with scroll-spy, wide hero with the
  mural on the right, 4-up dish cards, two-column menu, gallery carousel with
  arrows and peeking neighbours, and the order bar as a floating card.

Gallery supports swipe, click-drag, arrow keys, dots, autoplay (pauses on hover
or when off-screen), and click-to-enlarge. Respects `prefers-reduced-motion`.
